...
class SimpleMovingAverage(Indicator):
    ...
    def compute(self: Self, data: pd.DataFrame) -> list[str]:
        ...
        data[sma_column] = (
            data[self._column_name].rolling(window=self._window_size).mean()
        )
        return [sma_column]
...
class ExponentialMovingAverage(Indicator):
    ...
    def compute(self: Self, data: pd.DataFrame) -> list[str]:
        ...
        data[ema_column] = (
            data[self._column_name].ewm(span=self._ema_span, adjust=False).mean()
        )
        return [ema_column]
...
class RelativeStrengthIndex(Indicator):
    ...
    def compute(self: Self, data: pd.DataFrame) -> list[str]:
        ...
        for i in range(self._window_size + 1, len(data)):
            avg_gain.iloc[i] = (
                avg_gain.iloc[i - 1] * (self._window_size - 1) + gain.iloc[i]
            ) / (self._window_size)
            avg_loss.iloc[i] = (
                avg_loss.iloc[i - 1] * (self._window_size - 1) + loss.iloc[i]
            ) / (self._window_size)
...
class MovingAverageConvergenceDivergence(Indicator):
    def compute(self, data: pd.DataFrame) -> list[str]:
        ...
        data[ema_12_column] = (
            data["Close"].ewm(span=12, adjust=False, min_periods=12).mean()
        )
        data[ema_26_column] = (
            data["Close"].ewm(span=26, adjust=False, min_periods=26).mean()
        )
