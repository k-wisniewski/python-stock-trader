...
def test_macd_strategy(sample_data_from_file: pd.DataFrame) -> None:
    ...
    assert sample_data_from_file[macd_columns[0]].iloc[-1] == pytest.approx(
        4.979, rel=1e-3
    )
    assert sample_data_from_file[macd_columns[1]].iloc[-1] == pytest.approx(
        3.694, rel=1e-3
    )
    assert sample_data_from_file[macd_columns[2]].iloc[-1] == pytest.approx(
        1.284, rel=1e-3
    )
