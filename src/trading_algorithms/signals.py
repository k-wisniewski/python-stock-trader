...
class MovingAverageCrossoverSignal(Signal):
    ...
    def generate_signals(self: Self, data: DataFrame) -> str:
        ...
        return self._calculate_buy_sell_signal_from_indicators(
            data, short_ema_column, long_sma_column
        )
    ...
    def _calculate_buy_sell_signal_from_indicators(
        ...
        ma_crossover_column = (
            f"Signal_EMA{self._short_window_days}_SMA{self._long_window_days}"
        )
        data.loc[
            data[short_ema_column] > data[long_sma_column], ma_crossover_column
        ] = 1.0
        data.loc[
            data[short_ema_column] <= data[long_sma_column], ma_crossover_column
        ] = -1.0
    ...
    def _calculate_indicators(self: Self, data: pd.DataFrame) -> tuple[str, str]:
        short_ema_column = ExponentialMovingAverage(
            window_size=self._short_window_days
        ).compute(data)
        long_sma_column = SimpleMovingAverage(
            window_size=self._long_window_days
        ).compute(data)
...
class RelativeStrengthIndexSignal(Signal):
    ...
    def generate_signals(self: Self, data: pd.DataFrame) -> str:
        ...
        rsi_column = RelativeStrengthIndex(window_size=self._window_size).compute(data)[
            0
        ]
        ...
        data.loc[
            (30 <= data[rsi_column]) & (data[rsi_column] <= 70), rsi_signal_column
        ] = 0.0
...
class MovingAverageConvergenceDivergenceSignal(Signal):
    ...
    def generate_signals(self: Self, data: pd.DataFrame) -> str:
        ...
        data.loc[
            data["MACDLine"] < data["SignalLine"], macd_column
        ] = -1.0  # Sell signal
        return macd_column
