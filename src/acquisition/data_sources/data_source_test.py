...
import pandas as pd
import pytest
from acquisition.data_sources.alpha_vantage import AlphaVantageDataSource
from acquisition.data_sources.data_source import DataSource

from acquisition.data_sources.local_csv import LocalCSVDataSource, TickerNotFoundError
from acquisition.data_sources.yahoo_finance import YFinanceDataSource
...
DATA_SOURCES = {
    ...
    AlphaVantageDataSource("KMCTPR2AV0KNKXVX"): DateRange(
        start=datetime(2023, 7, 1), end=datetime(2023, 9, 20)
    ),
    YFinanceDataSource(): DateRange(
        start=datetime(2023, 7, 1), end=datetime(2023, 9, 20)
    ),
}
...
def test_correct_columns_are_returned(data_source: DataSource):
    df = data_source.fetch(
        "ge", DateRange(start=datetime(2007, 11, 9), end=datetime(2017, 11, 9))
    )
