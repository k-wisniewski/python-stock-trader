...
class Visualizer(ABC):
    ...
    def plot(
        self: Self, data: pd.DataFrame, ticker: str, indicator_names: list[str]
    ) -> None:
        subplots = self._gather_all_subplots(data, ticker, indicator_names)
        if len(subplots) == 0:
            self._plot_stock_price(data, type="candle", style="yahoo", volume=True)
        else:
            self._plot_stock_price_with_indicator(
                data, ticker, indicator_names, subplots
            )
        self._save_and_cleanup(ticker, indicator_names)

    def _gather_all_subplots(
        self: Self, data: pd.DataFrame, ticker: str, indicator_names: list[str]
    ) -> list[Subplot]:
    ...
    def _plot_stock_price(
        self: Self, data: pd.DataFrame, type: str, style: str, volume: bool
    ) -> None:
        mpf.plot(
    ...
    def _plot_indicator(
        self: Self, data: pd.DataFrame, ticker: str, indicator_name: str, subplots: list
    ) -> None:
        if self._can_draw_indicator(indicator_name):
            ...
            self._next_visualizer._plot_indicator(
                data, ticker, indicator_name, subplots
            )
    ...
    def _save_and_cleanup(self: Self, ticker: str, indicator_names: list[str]) -> None:
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        plt.savefig(
            APP_SETTINGS.report_output_path
            / f"{ticker}_{'_'.join(indicator_names)}_{timestamp}.png"
        )
        plt.close()
...
class MACDVisualizer(Visualizer):
    ...
    def _get_subplot(self: Self, data: pd.DataFrame, indicator_name: str) -> Subplot:
        if indicator_name == "MACDHistogram":
            ...
            return mpf.make_addplot(
                data["MACDLine"], panel=1, color="fuchsia", secondary_y=False
            )
        elif indicator_name == "SignalLine":
            return mpf.make_addplot(
                data["SignalLine"], panel=1, color="b", secondary_y=False
            )
        raise ValueError(
            f"MACDVisualizer: Unsupported indicator name: {indicator_name}"
        )
