...
def test_rsi_visualizer_outputs_png_image_in_the_report_output_path(
    ...
    data = fake_data_source.load_to_dataframe("AAPL")
    data["RSI_14"] = pd.Series(
        [47.0, 31.0, 28.0, 37.0, 52.0], index=data.index, name="RSI_14"
    )
...
def test_macd_visualizer_outputs_png_image_in_the_report_output_path(
    ...
    data = fake_data_source.load_to_dataframe("AAPL")
    data["MACDLine"] = pd.Series(
        [47.0, 31.0, 28.0, 37.0, 52.0], index=data.index, name="MACDLine"
    )
    data["SignalLine"] = pd.Series(
        [47.0, 31.0, 28.0, 37.0, 52.0], index=data.index, name="SignalLine"
    )
    data["MACDHistogram"] = pd.Series(
        [47.0, 31.0, 28.0, 37.0, 52.0], index=data.index, name="MACDHistogram"
    )
    visualizer.plot(data, "AAPL", ["MACDLine", "SignalLine", "MACDHistogram"])
    assert (
        Path(tmp_path)
        / Path("AAPL_MACDLine_SignalLine_MACDHistogram_2020-01-01_00-00-00.png")
    ).exists()
...
def test_sma_visualizer_outputs_png_image_in_the_report_output_path(
    ...
    data = fake_data_source.load_to_dataframe("AAPL")
    data["SMA_200"] = pd.Series(
        [47.0, 31.0, 28.0, 37.0, 52.0], index=data.index, name="SMA_200"
    )
...
def test_ema_visualizer_outputs_png_image_in_the_report_output_path(
    ...
    data = fake_data_source.load_to_dataframe("AAPL")
    data["EMA_200"] = pd.Series(
        [47.0, 31.0, 28.0, 37.0, 52.0], index=data.index, name="EMA_200"
    )
