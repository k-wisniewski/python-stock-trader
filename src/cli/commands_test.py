...
def test_date_range_can_be_set_with_options_start_and_end(
    runner: CliRunner, tickers_path: Path
) -> None:
...
def test_date_range_can_be_set_with_last_option(
    runner: CliRunner, tickers_path: Path
) -> None:
    result = runner.invoke(
        cli,
...
def test_when_alpha_vantage_specified_api_key_required(
    runner: CliRunner, tickers_path: Path
) -> None:
    result = runner.invoke(
        cli,
...
def test_conditionally_options_are_not_required_when_conditions_not_met(
    runner: CliRunner, tickers_path: Path
) -> None:
