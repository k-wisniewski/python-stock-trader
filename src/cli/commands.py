...
from workflows.backtesting import BacktestingWorkflow
from trading_algorithms.factory import indicator_factory, signal_factory
...
def cli(
    ...
    data_src = None
    date_range = (
        DateRange(start=start, end=end) if not last else DateRange.from_last(last)
    )
    if data_source == "alpha_vantage":
        alpha_vantage_api_key = alpha_vantage_api_key or str(
            click.prompt("Alpha Vantage API key")
        )
